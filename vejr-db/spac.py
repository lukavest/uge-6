...
token = os.getenv("SPAC_API_TOKEN")
headers = {"Authorization": f"Bearer {token}"}

source_ids = ["BME280", "DS18B20"]
table_names = {src_id: src_id.lower() + "_readings" for src_id in source_ids}
shared_cols = [("id", "CHAR(36) PRIMARY KEY"), ("timestamp", "TIMESTAMPTZ"), ("source_id", "VARCHAR(16)"),
               ("temperature", "FLOAT")]
...
class SpacClient:
    ...
    def create_table(self, src_id, drop_first=False):
    ...
    def insert_records(self, src_id, records_lst, ignore_conflict=True, commit=False):
        if src_id not in source_ids:
            raise ValueError("Invalid source id")
        ...
        self.conn.cur.executemany(q, records_lst)
        if commit:
            self.conn.commit()

    def update_table(self, src_id):
        table_name = table_names[src_id]
        ...
        while n_records == max_limit:
            try:
                mx = self.conn.query_fetch(f"SELECT MAX(timestamp) FROM {table_name}")[0][0]
            ...
            if n_records > 0:
                self.insert_records(src_id, records, commit=True)

    def create_all(self, drop_first=False):
        for source_id in source_ids:
            self.create_table(source_id, drop_first)

    def update_all(self):
        for source_id in source_ids:
            self.update_table(source_id)


def main():
    conn = Connection()
    spac = SpacClient(conn)
    spac.create_all()
    spac.update_all()
    conn.commit()
    conn.close()
